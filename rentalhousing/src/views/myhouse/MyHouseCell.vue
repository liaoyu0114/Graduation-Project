      ...
      <el-col :span="24" v-if="userInfo.tenant_id">
        <el-card class="box-card">
          <div slot="header" class="clearfix">
            <span class="house-title">租房订单ID: {{lease.lease_id}}</span>
            <span class="house-title">创建时间: {{lease.lease_time | formatDate("YYYY-MM-DD HH:mm:ss")}}</span>
            <span class="lease-state leasing" v-if="lease.lease_type === 0">租赁中</span>
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="18">
              <el-row>
                <el-col :span="24">
                  <div class="user-info">
                    <el-avatar :src="lease.landlord.landlord_pic" :size="50"></el-avatar>
                    <div class="user-info-name">{{lease.landlord.landlord_nickname}}</div>
                  </div>
                </el-col>
                <el-col :span="24">
                  <div class="cell-describe">
                    <div class="des-box">
                      ...
                      <span class="vr-line">{{lease.house.housingresources_area}}平米</span>
                      <span class="vr-line">{{lease.house.housingresources_floor}}</span>
                    </div>

                    <div class="dec-address">
                      <span class="vr-line">{{lease.house.housingresources_village}}</span>
                      <span class="vr-line">{{lease.house.housingresources_address}}</span>
                    </div>
                  </div>
                </el-col>
                <el-col :span="4" :offset="20" class="price-box">
                  <div class>
                    <span class="price">{{lease.house.housingresources_price}}</span>
                    <span>元/每月</span>
                  </div>
                </el-col>
              </el-row>
            ...
            <el-col :span="12" :offset="12">
              <div class="button-area">
                ...
                <el-button v-if="lease.lease_type === 0" type="danger" size="mini" plain @click="applyCancel">申请退房</el-button>
              </div>
            </el-col>
          </el-row>
        </el-card>
...
export default {
  ...
  computed: {
    ...mapGetters(["userInfo"]),
    getHType() {
      return `${this.lease.house.housingresources_type.first}室${this.lease.house.housingresources_type.second}厅${this.lease.house.housingresources_type.third}卫`;
    }
  },
  methods: {
    showLandlord() {
      this.$confirm(
        `房东电话:${this.lease.landlord.landlord_contact}`,
        "提示",
        {
          type: "info",
